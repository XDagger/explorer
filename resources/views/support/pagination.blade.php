	...
	<div class="w-full mt-8 -mb-2">
		<div class="flex flex-wrap items-start justify-between">
			<div class="flex-order-1 w-1/2 sm:w-auto pr-1 sm:pr-0">
				<a href="{{ $paginator->previousPageUrl() }}" rel="nofollow" class="flex items-center justify-center button secondary small {{ $paginator->onFirstPage() ? 'disabled' : '' }}">Previous</a>
			</div>
			...
			<div class="flex-order-2 sm:flex-order-3 w-1/2 sm:w-auto pl-1 sm:pl-0">
				<a href="{{ $paginator->nextPageUrl() }}" rel="nofollow" class="flex items-center justify-center button secondary small {{ $paginator->currentPage() == $paginator->lastPage() ? 'disabled' : '' }}">Next</a>
			</div>
		</div>
	</div>
@endif
